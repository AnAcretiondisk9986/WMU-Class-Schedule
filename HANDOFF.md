...
页面也支持从教务系统获取 PDF。`src/jwxt-bridge.js` 生成限定目标来源的书签脚本：用户由应用打开教务系统并登录后，在课表页点击该书签，脚本用页面已有的 `downForm` 同源请求 PDF，再通过 `postMessage` 返回应用并自动解析。接收端校验 `jwxt.wmu.edu.cn` 来源、最初打开的窗口、32 MiB 大小上限和 `%PDF-` 文件签名。纯静态站点无法跨域注入登录后的教务系统页面，因此无扩展方案仍需要用户点击一次书签；原有本地文件导入保留。

### 两套校区时间

每节课 40 分钟，课间 5 分钟，第 `n` 节的开始时间为：
...
| 学院路校区 | 08:00 | |
| 滨海校区 | 08:30 | |
| 线上 | 08:30 | 仅作线上课程显示，不参与物理校区冲突 |

课表 Y 轴从 08:00 起按 45 分钟一档形成严格时间标尺；课程卡片按实际开始/结束分钟定位与定高，并显示该课程所属校区的精确时间段（例如滨海校区显示 `08:30–09:55`）。
