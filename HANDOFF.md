...
页面也支持从教务系统获取 PDF。`src/jwxt-bridge.js` 生成限定目标来源的脚本：用户由应用打开教务系统并登录后，回到应用点击「已登录，获取课表」，脚本通过命名窗口在 `jwxt.wmu.edu.cn` 同源上下文提交页面已有的 `downForm`，再通过 `postMessage` 返回应用并自动解析。手机浏览器不支持该跨窗口脚本导航时，可将应用安装到主屏幕；教务系统导出的 PDF 通过系统分享进入 PWA Share Target，由 `sw.js` 暂存在 IndexedDB，再由 `src/shared-pdf.js` 取出解析。接收端校验 `jwxt.wmu.edu.cn` 来源、最初打开的窗口、32 MiB 大小上限和 `%PDF-` 文件签名。所有路径都使用正常页面导航、用户认证和系统分享，不读取 Cookie、不监听网络流量、不绕过认证；原有本地文件导入保留。
